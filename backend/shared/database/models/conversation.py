...
class ConversationORM(Base):
    ...
    conversation_metadata: Mapped[Optional[str]] = mapped_column(JSON, nullable=True, comment="元数据JSON")
    ...
    __table_args__ = (
        ...
        CheckConstraint("temperature >= 0 AND temperature <= 2", name="check_conversation_temperature_range"),
...
class MessageORM(Base):
    ...
    # 元数据
    message_metadata: Mapped[Optional[str]] = mapped_column(JSON, nullable=True, comment="元数据JSON")
...
class MessageAttachmentORM(Base):
    ...
    # 元数据
    attachment_metadata: Mapped[Optional[str]] = mapped_column(JSON, nullable=True, comment="元数据JSON")
