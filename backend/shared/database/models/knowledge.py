...
class KnowledgeDocumentORM(Base):
    ...
    # 元数据
    document_metadata: Mapped[Optional[str]] = mapped_column(JSON, nullable=True, comment="元数据JSON")
...
class DocumentChunkORM(Base):
    ...
    # 元数据
    chunk_metadata: Mapped[Optional[str]] = mapped_column(JSON, nullable=True, comment="元数据JSON")
    keywords: Mapped[Optional[str]] = mapped_column(JSON, nullable=True, comment="关键词JSON")
...
document_entity_associations = Table(
    ...
    Base.metadata,
    Column('document_id', CHAR(36), primary_key=True),
    Column('entity_id', CHAR(36), primary_key=True),
